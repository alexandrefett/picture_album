...
class FireService implements BaseDB{
  ...
  @override
  Stream<fs.QuerySnapshot> getCatFromHotel(String id) {
    return store.collection('hoteis').doc(id).collection('categorias').onSnapshot;
  }
  ...
  @override
  Future deleteCat(String hid, String id) {
    return store.collection('hoteis').doc(hid).collection('categorias').doc(id).delete();
  }

  @override
  Future addHotel(Hotel hotel, Blob image) async {
    String filename = 'album/'+DateTime.now().millisecondsSinceEpoch.toString();
    fbstorage
        .ref(filename)
        .put(image)
        .future
  ...
  Future addCat(Categoria cat, String hotelid, Blob image) async {
    ...
    fbstorage
      ...
      getUrlFromFile(filename).then((onData){
        ...
        store.collection('hoteis').doc(hotelid).collection('categorias').add(cat.toJson()).then((data){
          store.collection('hoteis').doc(hotelid).collection('categorias').doc(data.id).update(data: {'id':data.id});
        });
      });
    });
  }
  ...
  Future updateHotel(Hotel hotel) async {
    store.collection('hoteis').where('nome','==',hotel.nome).onSnapshot.first.then((querysnapshot){
      store.collection('hoteis').doc(querysnapshot.docs[0].id).update(data: hotel.toJson());
    });
  }
